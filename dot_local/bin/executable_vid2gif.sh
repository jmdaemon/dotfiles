...
# https://github.com/lukechilds/vid2gif
show_help() {
  echo "vid2gif 0.1.0"
  echo
  echo "Usage: vid2gif [options] [input]"
  echo
  echo "Options:"
  ...
  echo
  echo "Examples:"
  echo "  $ vid2gif video.mp4"
  echo "  $ vid2gif -o demo.gif SCM_1457.mp4"
  echo "  $ vid2gif -sf 15 screencap.mov"
  echo "  $ vid2gif -sf 15 -w 320 screencap.mov"
  echo
  echo "Begin at 3.5 seconds into the video, make the gif using the next 5.5 seconds"
  echo "  $ vid2gif -b 3.5 -d 5.5 screencap.mov"
}
...
type ffmpeg >/dev/null 2>&1 || {
  echo "Error: vid2gif requires ffmpeg to be installed"
  exit 1
}
